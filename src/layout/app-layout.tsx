...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiFileText } from 'react-icons/fi';
...
import { getOrganizers } from 'apiSdk/organizers';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['personal-app-owner', 'collaborator', 'guest'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<OrganizerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/organizers` : null),
    () => getOrganizers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Organizer',
      path: isTenantUser ? `/organizers/view/${data?.[0]?.id}` : '/organizers',
      entity: 'organizer',
      service: AccessServiceEnum.PROJECT,
      icon: FiUsers,
    },
    { name: 'Template', path: '/templates', entity: 'template', service: AccessServiceEnum.PROJECT, icon: FiFileText },
